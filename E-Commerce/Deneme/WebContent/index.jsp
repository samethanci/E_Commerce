	...
	<div class="container">
		<div class="row py-3">
			<c:forEach var="urun"  items="${urunListesi}">
				<div class="col-lg-4 text-center border border-dark text-light bg-secondary p-3 m1">
					<p><c:out value="${urun.adi}"/> : <c:out value="${urun.fiyat}"/>TL</p>
					<%if(e_posta != null) {%>
						<form action="sepeteEkle" method="post">
							<input type="hidden" name="urun_id" value="${urun.urun_id}">
							<input type="submit" class="btn btn-outline-light" value="Sepete Ekle">
						</form>
					<%} %>
				</div>
			</c:forEach>
